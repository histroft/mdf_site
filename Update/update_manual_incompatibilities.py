...
def show_manual_incomp_stats(db_file):
    """
    Показывает статистику по таблице Manual_incomp
    """
    conn = sqlite3.connect(db_file)
    cursor = conn.cursor()
    ...
    try:
        # Общая статистика
        cursor.execute("SELECT COUNT(*) FROM Manual_incomp")
        total = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM Manual_incomp WHERE resolution = 1")
        allowed = cursor.fetchone()[0]
        
        cursor.execute("SELECT COUNT(*) FROM Manual_incomp WHERE resolution = 0")
        forbidden = cursor.fetchone()[0]
        
        print(f"\n📊 СТАТИСТИКА Manual_incomp:")
        print(f"   Всего записей: {total}")
        print(f"   Разрешено (1): {allowed}")
        print(f"   Запрещено (0): {forbidden}")
        ...
        print(f"\n🔝 Топ-10 моделей по количеству правил:")
        for model, count in cursor.fetchall():
            print(f"   {model}: {count} правил")
...
if __name__ == "__main__":
    ...
    if success:
        ...
        show_manual_incomp_stats(db_name)