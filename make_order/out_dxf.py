"""
Модуль для создания DXF чертежа наружной стороны двери
Версия 2.0 - все линии становятся черными в конце
"""
import logging
...
class MakeOutDXF:
    """
    Класс для создания DXF чертежа наружной стороны двери
    """
    ...
    # Базовая ширина и высота для масштабирования
    BASE_WIDTH = 950
    BASE_HEIGHT = 2050
    ...
    # Ширина для вставки рисунка
    PIC_BASE_WIDTH = 900
    PIC_OFFSET = PIC_BASE_WIDTH // 2
    
    # Пути к ресурсам (относительно текущего модуля)
    PIC_DIR = "Pic"
    FURNITURE_DIR = "Furniture"
    MAIN_LOCK_DIR = "main_lock/out"
    ...
    def _extract_data(self):
        ...
        self.out_pic = self.data.get('05_Лицо (рисунок)', '').replace('/', '+')
        ...
        logger.debug(f"Рисунок: {self.out_pic}, Сторона: {self.side}")
    ...
    def _setup_paths(self):
        ...
        self.base_file = self.abs_path / 'DXF' / self.model / f'{self.model}_OUT.dxf'
    ...
    def _get_pic_file_path(self) -> Path:
        ...
        # Специальные случаи
        if self.out_pic.startswith("S11"):
            return pic_dir / 'S11_out.dxf'
        if self.out_pic.startswith("S12"):
            return pic_dir / 'S12_out.dxf'
        if self.out_pic.startswith("НСТ_16_"):
            pic_name = self.out_pic.replace('НСТ_16_', '') + '_out.dxf'
        ...
        for file_name in union_files:
            if file_name in self.out_pic:
                return pic_dir / f'{file_name}_out.dxf'
        
        # По умолчанию
        return pic_dir / f'{self.out_pic}_out.dxf'
    ...
    def build(self):
        ...
        logger.info("🏗️ Начало построения наружного вида")
        
        # ШАГ 0: Очистка чертежа от лишнего (если нужно)
        self.doc.analyze_drawing() # type: ignore
        # self.doc.clean_drawing()  # если нужно удалить лишние типы
    ...
    def _insert_picture(self):
        """Вставляет рисунок на дверь с принудительным смещением"""
        if not self.out_pic or not self.pic_file.exists():
            return
        
        # Принудительное смещение - вставляем рисунок так, чтобы его левый нижний угол был в (0,0)
        offset = (450, 0, 0)  # Пробуем без смещения
        
        logger.info(f"🖼️ Вставка рисунка: {self.pic_file.name} со смещением {offset}")
    ...
    def _scale(self):
        ...
        # ИЗМЕНЕНО: метод scale уже есть в DXFDocument
        self.doc.scale(x_scale, y_scale)# type: ignore
        
    
    def _insert_furniture(self):
        """Вставляет фурнитуру на чертеж"""
        # Основной замок
        if self.lock_file.exists():
            offset = (self.LOCK_X, self.LOCK_Y, 0)
            # ИЗМЕНЕНО: insert_dxf вместо insert_into_msp
            success = insert_dxf(self.doc.msp, str(self.lock_file), offset)# type: ignore
        
            if success:
                logger.debug(f"   ✅ Основной замок добавлен")
        else:
            logger.warning(f"   ⚠️ Файл замка не найден: {self.lock_file}")
        
        # Дополнительный замок
        if self.adv_lock:
            if self.adv_lock_file.exists():
                offset = (self.LOCK_X, self.ADV_LOCK_Y, 0)
                success = insert_dxf(self.doc.msp, str(self.adv_lock_file), offset)# type: ignore
        
                if success:
                    logger.debug(f"   ✅ Дополнительный замок добавлен")
            else:
                logger.warning(f"   ⚠️ Файл доп. замка не найден: {self.adv_lock_file}")
    
    def _get_bounds(self): # type: ignore
    ...
    def _mirror_if_needed(self):
        """Зеркалирует чертеж для левой стороны"""
        if 'L' in self.side:
            self.doc.mirror(axis='y') # type: ignore
        
            logger.debug("🪞 Чертеж отзеркален для левой стороны")
    
    def _make_all_black(self):
        """
        Принудительно устанавливает черный цвет для всех сущностей в чертеже
        """
        logger.debug("⚫ Преобразование всех линий в черный цвет")
        
        changed = 0
        for entity in self.doc.msp: # type: ignore
        
            old_color = entity.dxf.get('color', 7)
            if old_color != 1:  # Если не черный
                entity.dxf.color = 1
                changed += 1
    ...
    def get_document(self) -> DXFDocument:
        """Возвращает документ для дальнейшего использования"""
...
if __name__ == '__main__':
    ...
    test_data = {
        ...
        '01_Ширина': 1000,
        '02_Высота': 2200,
        'path_to_order': str(Path.home() / 'NEW_NST_SERVER' / 'TEST'),
        '05_Лицо (рисунок)': 'D-L1',
        'peep': True,
        'peep_offset': False,
        'adv_lock': True,
    ...
    try:
        ...
        out = MakeOutDXF(test_data, Path.home() / 'NEW_NST_SERVER' / 'TEST')
        out.build()
        out.save()
        
        print(f"\n✅ Чертеж создан: {out.out_file}")
        print(f"   Границы: x=[{out.min_x:.1f}, {out.max_x:.1f}], y=[{out.min_y:.1f}, {out.max_y:.1f}]")
        print("="*60)
